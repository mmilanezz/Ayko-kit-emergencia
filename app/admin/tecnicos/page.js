...
export default function TecnicosPage() {
  ...
  async function cadastrarTecnico(e) {
    ...
    if (!resposta.ok) {
      setErro(resultado.error || "Erro ao cadastrar técnico.");
      return;
    }

    setUltimaSenha({ email: form.email, senha: form.senha });
    setForm({ nome: "", email: "", senha: gerarSenha(), dupla_id: "" });
    carregar();
  }
  ...
  async function excluirTecnico(id, nome) {
    ...
    if (!resposta.ok) {
      alert(resultado.error || "Erro ao excluir técnico.");
      return;
    }
    carregar();
  }
  ...
  return (
    <main className="pb-16">
      <TopBar titulo="Técnicos" subtitulo="Cadastro e gestão dos logins dos técnicos" />

      <section className="p-6">
        <h2 className="text-sm font-medium text-slate-400 mb-3">Cadastrar novo técnico</h2>
        <form
          onSubmit={cadastrarTecnico}
        ...
        >
          ...
          {ultimaSenha && (
            <div className="col-span-2 bg-green/10 border border-green/30 rounded-lg p-3 text-sm">
              <p className="text-green font-medium mb-1">Técnico cadastrado!</p>
          ...
          <div className="col-span-2">
            ...
            >
              {enviando ? "Cadastrando..." : "Cadastrar técnico"}
            </button>
          </div>
        </form>
      </section>
      ...
      <section className="px-6">
        <h2 className="text-sm font-medium text-slate-400 mb-3">
          Técnicos cadastrados {!carregando && `(${tecnicos.length})`}
        </h2>
                ...
                <tr key={t.id} className="border-b border-border last:border-0">
                  <td className="px-4 py-3">
                    <input
                      type="text"
                      defaultValue={t.nome}
                      onBlur={(e) => atualizarCampo(t.id, "nome", e.target.value)}
                  ...
                  <td className="px-4 py-3">
                    <select
                      value={t.dupla_id || ""}
                      onChange={(e) => atualizarCampo(t.id, "dupla_id", e.target.value || null)}
                  ...
                  <td className="px-4 py-3 text-right">
                    <button
                      onClick={() => excluirTecnico(t.id, t.nome)}
